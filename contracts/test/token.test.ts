...
describe('Token', function () {
  async function deployFixture() {
    ...
    const Token1155A = await hre.ethers.getContractFactory('Token');
    const token1155A = await Token1155A.deploy(owner);
    await token1155A.mint(owner, tokenType, tokenAmount, nullData);
    return {
      token: token1155A,
      tokenType,
      tokenAmount,
  ...
  describe('Deployment', function () {
    ...
    it('Should allow owner to pause and unpause the contract', async function () {
      const { token, owner, tokenType, tokenAmount, nullData } =
        await loadFixture(deployFixture);
      await token.pause();
      await expect(
        token.mint(owner, tokenType, tokenAmount, nullData)
      ).to.be.revertedWithCustomError(token, 'EnforcedPause');
      await token.unpause();
      await token.mint(owner, tokenType, tokenAmount, nullData);
  ...
  describe('Mint', function () {
    it('Should mint the right value', async function () {
      ...
      await token.mint(owner, tokenType, tokenToMint, nullData);
    ...
    it('Should be able to call mint as owner for another account', async function () {
      const { token, otherAccount, tokenType, tokenAmount, nullData } =
        await loadFixture(deployFixture);

      const oldValue = await token.balanceOf(otherAccount, tokenType);
      await token.mint(otherAccount, tokenType, tokenAmount, nullData);
    ...
    it('Should revert with the right error if mint is called from another account', async function () {
      ...
      await expect(
        token
          ...
          .mint(otherAccount, tokenType, tokenAmount, nullData)
  ...
  describe('Transfert', function () {
    it('Should be able to transfer my tokens', async function () {
      const { token, owner, tokenType, otherAccount, tokenAmount } =
        await loadFixture(deployFixture);
    ...
    it('Should receive the right error if I try to transfer an amount of tokens I do not possess', async function () {
      const { token, owner, otherAccount, tokenType, tokenAmount } =
        await loadFixture(deployFixture);
