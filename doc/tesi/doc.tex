...
\subsubsection{Definizione del modello di dominio}
...
L'autenticazione degli utenti è gestita dal server autoritativo Keycloak. In un contesto OAuth2 il componente Spring riveste il ruolo di \textit{resource server}, Keycloak è l' \textit{authentication server}, le risorse sono le informazioni veicolate tramite le chiamate alle API\footnote{Application Programming Interface - per una lista degli endpoints disponibili vedere \ref{}} ed in fine la componente d'interfaccia, che viene eseguita nel browser dell'utente, è il client.
...
L'implementazione della base di dati è stata
...
\begin{listing}[H]
  \inputminted{javascript}{./code/hardhat.js}
  \caption{Contratto di notarizzazione - }
  \label{listing:hash3}
\end{listing}
